...
public class LBRServer extends RoboticsAPIApplication
{
  ...
  // CartesianImpedanceControlMode parameters
    private double[] cart_stiffness_ = {1000.0, 1000.0, 1000.0,
                                        30.0, 30.0, 30.0};
    private double cart_dumping_     = 1.0;
    private double ns_stiffness_     = 30.0;
    private double ns_dumping_       = 1.0;
  ...
  // methods
    ...
    {
        fri_configuration_
            = FRIConfiguration.createRemoteConfiguration(lbr_, client_name_);
        fri_configuration_.setPortOnRemote(client_port_);
        fri_configuration_.setSendPeriodMilliSec(send_period_);
        getLogger().info("Creating FRI connection to "
                         + fri_configuration_.getHostName());
        getLogger().info("SendPeriod: "
                         + fri_configuration_.getSendPeriodMilliSec()
                         + "ms |"
                         + " ReceiveMultiplier: "
                         + fri_configuration_.getReceiveMultiplier());
        ...
        fri_session_.addFRISessionListener(
            new IFRISessionListener()
            {
                    ...
                    FRIChannelInformation friChannelInformation)
                {
                    ...
                    getLogger().info("Jitter "
                                     + friChannelInformation.getJitter());
                    getLogger().info("Latency "
    ...
    {
      // run the FRI
        lbr_.move(positionHold(control_mode_, -1, TimeUnit.SECONDS)
